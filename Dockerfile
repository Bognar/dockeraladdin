...
RUN apt-get update && apt-get install -y tightvncserver ratpoison dosbox novnc websockify && \
 ...
 openssl req -x509 -nodes -newkey rsa:2048 -keyout ~/novnc.pem -out ~/novnc.pem -days 3650 -subj "/C=US/ST=NY/L=NY/O=NY/OU=NY/CN=NY emailAddress=josip.bognar@gmail.com"
ENV PORT 6080
